...
async function buildRatingResponse(seriesId: string, userId?: string) {
  const stats = await getSeriesRatingStats(seriesId);
  const user_rating =
    userId !== undefined ? await getUserSeriesRating(userId, seriesId) : null;
  return {
...
export async function PUT(request: Request, { params }: RouteParams) {
  try {
    ...
    await upsertSeriesRating(user.id, id, rating);
...
export async function DELETE(_request: Request, { params }: RouteParams) {
  try {
    ...
    await deleteSeriesRating(user.id, id);
    return NextResponse.json(await buildRatingResponse(id, user.id));
  } catch (error) {
    return handleRouteError(error);
  }
