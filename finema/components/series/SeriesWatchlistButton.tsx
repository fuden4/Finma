...
import { addSeriesToWatchlist, removeSeriesFromWatchlist } from "@/lib/api-client";

interface SeriesWatchlistButtonProps {
  seriesId: string;
...
export function SeriesWatchlistButton({
  seriesId,
  initialInWatchlist,
  variant,
  user,
  ...
  async function handleClick(e: React.MouseEvent) {
    ...
    try {
      if (next) {
        await addSeriesToWatchlist(seriesId);
      } else {
        await removeSeriesFromWatchlist(seriesId);
      }
      onChange?.(next);
    } catch {
      setInWatchlist(!next);
