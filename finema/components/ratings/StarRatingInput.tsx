...
export function StarRatingInput({
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const stats = await rateMovie(movieId, pendingRating);
  ...
  async function handleClear() {
    ...
    setError(null);
    try {
      const stats = await removeMovieRating(movieId);
      setUserRating(null);
      setPendingRating(null);
      onRated?.(stats);
    } catch (err) {
    ...
    <div className={compact ? "space-y-2" : "space-y-3"}>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className={`px-4 py-1.5 rounded font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
            compact
              ? "text-sm bg-finema-accent text-white hover:bg-finema-accent/90"
              : "px-5 py-2 bg-finema-accent text-white hover:bg-finema-accent/90"
          }`}
        >
      ...
      {userRating === null && pendingRating !== null && (
        <p className="text-sm text-finema-muted">
          Selected: {pendingRating} stars — press Submit to save
        </p>
      )}
